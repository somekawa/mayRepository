...
public:
	Item();
	~Item();
	char* GetSetumei(const int& num);					// 指定されたアイテムの説明を取得する
	int GetCostMoney(const int& num);					// 指定されたアイテムの値段を取得する
	std::pair<int, ITEM> GetPair(const int& num);		// 指定されたitem_pairの情報を取得する
	...
	void SetPos(const int& num);						// アイテムの座標を設定する(補充で使う)
private:
	void Init(void);											// 初期化
